...
struct Socket {
    ...
    static func tcpForListen(port: in_port_t = 8080) throws -> CInt {
        ...
        if ( bind(s, &sock_addr, socklen_t(sizeof(sockaddr_in))) == -1 ) {
            release(s)
            throw lastErr("bind(...) failed.")
        }
        if ( listen(s, 20 /* max pending connection */ ) == -1 ) {
            release(s)
            throw lastErr("listen(...) failed.")
        }
        return s
    }
